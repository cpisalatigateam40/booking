        ...
        <div id="approved-bookings-table">
            <div class="flex justify-between items-center mb-4">
                ...
                <div class="text-sm text-gray-500">
                    Total: <span id="approved-count">{{ $approvedBookings->count() }}</span> booking disetujui
                </div>
            </div>
            <div id="approved-bookings-content">
                ...
                <div id="approved-bookings-list" class="overflow-x-auto">
                    <pre>{{ print_r($approvedBookings->toArray(), true) }}</pre>
                    ...
                    <table class="min-w-full bg-white border border-gray-200 rounded-lg">
                        ...
                        <tbody id="approved-bookings-tbody" class="divide-y divide-gray-200">
                            @forelse ($approvedBookings as $booking)
                                <tr>
                                    <td class="px-4 py-2 whitespace-nowrap">{{ \Carbon\Carbon::parse($booking->date)->translatedFormat('d F Y') }}</td>
                                    <td class="px-4 py-2 whitespace-nowrap">{{ $booking->start_time }} - {{ $booking->end_time }}</td>
                                    <td class="px-4 py-2 whitespace-nowrap">{{ $booking->rooms->name ?? '-' }}</td>
                                    <td class="px-4 py-2 whitespace-nowrap">{{ $booking->name }}</td>
                                    <td class="px-4 py-2 whitespace-nowrap">{{ $booking->department->name ?? '-' }}</td>
